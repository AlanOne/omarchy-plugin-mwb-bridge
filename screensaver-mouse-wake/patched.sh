...
# MWB_MOUSE_WAKE: also exit on mouse movement, not just keyboard input, so
# moving the cursor onto this screen (e.g. a software KVM switching control
# over) dismisses the screensaver without requiring a keypress.
last_cursor=$(hyprctl cursorpos 2>/dev/null)
...
while true; do
  ...
  while pgrep -t "${tty#/dev/}" -x ttfx >/dev/null; do
    if read -n1 -t 0.2 || ! screensaver_in_focus; then
      exit_screensaver
    fi
